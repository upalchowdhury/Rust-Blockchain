...
//based on a consensus algorithm and based on the network’s hash-power, so the network can guarantee to produce a new block in a certain amount of time.
const DIFFICULTY_PREFIX: &str = "00";// Slice is a refference from a contiguous collection without ownership
...
impl Blockchain {
    ...
    fn is_block_valid(&self, block: &Block, previous_block: &Block) -> bool {
        if block.previous_hash != previous_block.hash {
            ...
            return false;
        } else if !hash_to_binary_representation(&hex::decode(&block.hash).expect(
            "can decode from hex "),
                ).starts_with(DIFFICULTY_PREFIX)
    {
        ...
        warn! (
            "block with id: {} is not the next block after the latest: {} ",block.id, previous_block.id
        );
        return false;
    ...
    // Always the longest chain is the valid chain
